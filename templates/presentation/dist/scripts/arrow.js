...
function svgSpotlight(x, y, r, gamma, color) {
  var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"); 
  var svgNS = svg.namespaceURI;
  ...
  circle.setAttribute('style', `fill:${color};stroke:black;stroke-width:0`);
...
$(document).ready(function () {
  ...
  $("div.spotlight").each(function (index) {
    ...
    var gamma = "50"
    if (this.hasAttribute('data-gamma')){
      gamma = this.getAttribute('data-gamma');
    } 

    var svg = svgSpotlight(x, y, r, gamma, color)
    if (this.classList.contains('fragment')){
      svg.classList.add('fragment');
    }
    this.parentNode.replaceChild(svg, this);
